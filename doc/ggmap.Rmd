...
ggmap(map, base_layer = ggplot(aes(x = lon, y = lat), data = new_campus)) + geom_point(aes(x=lon, y=lat, size = total), data = new_campus, alpha=.4) + stat_density2d(aes(x = lon, y = lat, fill=..level..), data = new_campus, geom="polygon", alpha=0.2) #+ scale_fill_gradient(low = "yellow", high = "red")
```
